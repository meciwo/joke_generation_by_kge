...
class TransEModel(TranslationModel):
    ...
    * Antoine Bordes, Nicolas Usunier, Alberto Garcia-Duran, Jason Weston, and
    ...
    def __init__(
        ...
        if ent_emb is None:
            self.ent_emb = init_embedding(self.n_ent, emb_dim)
        else:
            self.ent_emb = ent_emb
        if rel_emb is None:
            self.rel_emb = init_embedding(self.n_rel, emb_dim)
        else:
...
class TransHModel(TranslationModel):
    ...
    * Zhen Wang, Jianwen Zhang, Jianlin Feng, and Zheng Chen.
    ...
    def __init__(self, emb_dim, n_entities, n_relations):
        ...
        self.projected_entities = Parameter(
            empty(size=(self.n_rel, self.n_ent, self.emb_dim)), requires_grad=False
        )
    ...
    def scoring_function(self, h_idx, t_idx, r_idx):
        ...
        return -self.dissimilarity(
            self.project(h, norm_vect) + r, self.project(t, norm_vect)
        )
...
class TransRModel(TranslationModel):
    ...
    * Yankai Lin, Zhiyuan Liu, Maosong Sun, Yang Liu, and Xuan Zhu.
    ...
    def __init__(self, ent_emb_dim, rel_emb_dim, n_entities, n_relations):
        ...
        self.evaluated_projections = False
        self.projected_entities = Parameter(
            empty(size=(self.n_rel, self.n_ent, self.rel_emb_dim)), requires_grad=False
        )
    ...
    def scoring_function(self, h_idx, t_idx, r_idx):
        ...
        t = normalize(self.ent_emb(t_idx), p=2, dim=1)
        r = self.rel_emb(r_idx)
        proj_mat = self.proj_mat(r_idx).view(b_size, self.rel_emb_dim, self.ent_emb_dim)
        return -self.dissimilarity(
            self.project(h, proj_mat) + r, self.project(t, proj_mat)
        )
    ...
    def get_embeddings(self):
        ...
        return (
            self.ent_emb.weight.data,
            self.rel_emb.weight.data,
            self.proj_mat.weight.data.view(-1, self.rel_emb_dim, self.ent_emb_dim),
        )
    ...
    * Guoliang Ji, Shizhu He, Liheng Xu, Kang Liu, and Jun Zhao.
    ...
    def get_embeddings(self):
        ...
        return (
            self.ent_emb.weight.data,
            self.rel_emb.weight.data,
            self.ent_proj_vect.weight.data,
            self.rel_proj_vect.weight.data,
        )
    ...
    def lp_evaluate_projections(self):
        ...
        for i in tqdm(
            range(self.n_ent),
            unit="entities",  # TODO change this
